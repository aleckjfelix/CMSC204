...
public class DonationMgmGui extends Application {  
		...
		else 
		{
			try {
				...
				for (int i =0, j=t.length-1; i<t.length; i++, j--) {
					packages[i].setText(((DonationPackage) t[i]).toString());	
					  System.out.println((DonationPackage) t[i]);
				}
				System.out.println("------");
	...
	public void updateScreen() {
		...
		for (int i = 0; i < tempArr.length; i++) {
			packages[i].setText(((DonationPackage) tempArr[i]).toString());
	...
	public void donate() {
			...
			try {
				donationMgm.donatePackage();
					...
					System.out.println(donationMgm.toString());
			} catch (VolunteerException e) {
				// TODO Auto-generated catch block
				...
				System.out.println(donationMgm.container.containerEmpty() + "," + donationMgm.v_line.volunteerLineEmpty() + ","+ donationMgm.r_line.recipientLineEmpty());
				System.out.println("\n\n");
			}
	}
